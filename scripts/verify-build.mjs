...
const required = ['index.html', 'manifest.webmanifest', 'sw.js', 'icon.svg', 'config.js']
...
const publicConfig = fs.readFileSync(path.join(root, 'public', 'config.js'), 'utf8')
const configuredExec = publicConfig.match(/https:\/\/script\.google\.com\/macros\/s\/[A-Za-z0-9_-]+\/exec/)
if (!configuredExec) throw new Error('public/config.js must contain the deployed Web App /exec URL for this single-store build')
