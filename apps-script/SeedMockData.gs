function seedMockData() {
  ...
  return withScriptLock_(function() {
    ...
    products.forEach(function(item,index){
      item.shippingFee=index%3===0?45:index%3===1?35:25;item.shippingCalculation=index%2===0?'FLAT':'PER_UNIT';item.contentEnabled=true;
      var image=jsonArray_(item.imagesJson)[0]||'';item.contentJson=JSON.stringify([
        {id:item.id+'-story-h1',type:'h1',textTh:'เรื่องราวของ '+item.titleTh,textEn:'The story of '+item.titleEn,align:'left'},
        {id:item.id+'-story-p1',type:'p',textTh:'ตัวอย่างบทความสินค้าใน Google Sheet แสดงรายละเอียด จุดเด่น และวิธีดูแล เพื่อให้ลูกค้าตัดสินใจได้ง่ายขึ้น',textEn:'This sample product story is stored in Google Sheets and explains highlights, care, and useful buying details.',align:'left'},
        {id:item.id+'-story-image',type:'image',imageRef:image,align:'center'},
        {id:item.id+'-story-h2',type:'h2',textTh:'สิ่งที่ควรรู้ก่อนสั่งซื้อ',textEn:'Before you order',align:'left'},
        {id:item.id+'-story-p2',type:'p',textTh:item.type==='PREORDER'?'สินค้านี้เป็น Pre-order โปรดตรวจวันปิดรอบ วันคาดว่าถึง และเงื่อนไขการมัดจำก่อนยืนยัน':'สินค้าพร้อมส่งจะถูกพักสต็อก 20 นาทีหลังยืนยันออเดอร์ กรุณาส่งหลักฐานภายในเวลา',textEn:item.type==='PREORDER'?'This is a pre-order item. Check the closing time, expected arrival, and deposit terms before confirming.':'Ready stock is reserved for 20 minutes after checkout. Submit payment proof in time.',align:'left'}
      ]);
    });
    products.forEach(function(item) { upsertMock_('Products', item); });

    var announcements=[
      { id:'mock-ann-preorder', headerTh:'Pre-order รอบใหม่เปิดแล้ว', headerEn:'A new pre-order round is open', bodyTh:'รอบเกาหลีและญี่ปุ่นเปิดรับจำนวนจำกัด ดูวันปิดรอบได้ในหน้าสินค้า', bodyEn:'Korea and Japan rounds are open in limited quantities.', imageUrl:'https://images.unsplash.com/photo-1528164344705-47542687000d?auto=format&fit=crop&w=1200&q=80', kind:'PREORDER', active:true, sortOrder:1,contentEnabled:true,productIdsJson:JSON.stringify(['mock-po-grape-glass','mock-po-blindbox','mock-po-tote']),contentJson:JSON.stringify([{id:'ann-po-h1',type:'h1',textTh:'สองรอบพิเศษ จำนวนจำกัด',textEn:'Two limited pre-order rounds',align:'left'},{id:'ann-po-p1',type:'p',textTh:'เลือกสินค้าที่เชื่อมโยงด้านล่างได้ทันที แต่ละรอบมีวันปิดและกำหนดมัดจำต่างกัน',textEn:'Shop linked products below. Each round has its own closing date and deposit.',align:'left'},{id:'ann-po-image',type:'image',imageRef:'https://images.unsplash.com/photo-1528164344705-47542687000d?auto=format&fit=crop&w=1200&q=80',align:'center'}]) },
      { id:'mock-ann-new', headerTh:'สินค้าใหม่พร้อมส่ง', headerEn:'New ready-stock arrivals', bodyTh:'คุกกี้ มัทฉะ และไอเท็มบิวตี้พร้อมส่งแล้ววันนี้', bodyEn:'Cookies, matcha and beauty items are ready to ship.', imageUrl:'https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=1200&q=80', kind:'NEW_PRODUCT', active:true, sortOrder:2,contentEnabled:true,productIdsJson:JSON.stringify(['mock-ready-cookie','mock-ready-matcha','mock-ready-lip']),contentJson:JSON.stringify([{id:'ann-new-h1',type:'h1',textTh:'พร้อมส่ง ไม่ต้องรอรอบ',textEn:'Ready to ship',align:'center'},{id:'ann-new-p1',type:'p',textTh:'รวมสินค้าใหม่ที่มีสต็อกจริง กดดูรายละเอียดหรือเพิ่มลงตะกร้าจากประกาศนี้ได้เลย',textEn:'Browse in-stock arrivals and add linked products directly from this announcement.',align:'center'}]) },
      { id:'mock-ann-general', headerTh:'ตัวอย่างประกาศทั่วไปและค่าส่ง', headerEn:'General notice and shipping example', bodyTh:'ค่าส่งเหมารวมตั้งได้จากตั้งค่าแอป ส่วนพื้นที่พิเศษคิดตามจริง', bodyEn:'Set cart-wide shipping in app settings; special areas are charged at actual cost.', imageUrl:'', kind:'GENERAL', active:true, sortOrder:3,contentEnabled:true,productIdsJson:'[]',contentJson:JSON.stringify([{id:'ann-general-h2',type:'h2',textTh:'ตรวจสอบพื้นที่ก่อนยืนยัน',textEn:'Check your delivery area',align:'left'},{id:'ann-general-p',type:'p',textTh:'ลูกค้าต้องตรวจที่อยู่และระบุว่าเป็นพื้นที่ทั่วไป ห่างไกล พื้นที่ท่องเที่ยว หรือเกาะอย่างถูกต้อง',textEn:'Customers must verify whether the address is standard, remote, tourist, or island.',align:'left'}]) }
    ];announcements.forEach(function(item) { upsertMock_('Announcements', item); });
    ...
    upsertMock_('Orders', { id:'mock-order-ready-001', reference:'RD-MOCK-1001', userId:'mock-customer-001', orderType:'READY', status:'PAYMENT_REVIEW', subtotal:577, shippingFee:50,shippingBaseFee:50,shippingAdjustment:0, amountDueNow:627, totalPaid:627, balanceDue:0, reservedUntil:iso(1), shippingJson:JSON.stringify({name:'ลูกค้าทดสอบ',phone:'0800000000',address:'123 ถนนสุขุมวิท กรุงเทพฯ',xAccount:'mock_customer',specialAreaType:'NONE',areaClassificationConfirmed:true,specialAreaCostAccepted:false}), termsVersion:'',shippingTermsVersion:1,shippingTermsAcceptedAt:iso(0),areaClassificationAcceptedAt:iso(0), paymentAccountId:'mock-bank-kbank', createdAt:new Date().toISOString() });
    ...
    upsertMock_('Reviews',{id:'mock-review-cookie-001',userId:'mock-customer-001',productId:'mock-ready-cookie',orderItemId:'mock-item-ready-001',rating:5,body:'คุกกี้หอมมาก แพ็กเรียบร้อย ตัวอย่างรีวิวจากข้อมูลจริงใน Sheet',status:'APPROVED',adminReply:'ขอบคุณสำหรับรีวิวนะคะ 💜'});
    setSettingsBulk_({shippingMode:'CART',cartShippingFee:50,shippingFee:50,shippingTermsVersion:1,lowStockThreshold:3});
