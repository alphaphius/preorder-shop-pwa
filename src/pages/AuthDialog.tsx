...
export function AuthDialog({ locale, api, onClose, onAuthenticated }: { locale: Locale; api?: ApiClient; onClose: () => void; onAuthenticated: (session: SessionInfo) => void }) {
  const [step, setStep] = useState<'email' | 'code'>('email'); const [email, setEmail] = useState(''); const [code, setCode] = useState(''); const [busy, setBusy] = useState(false); const [error, setError] = useState('')
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setBusy(true); setError(''); try {
    ...
    else { const session = api ? await api.verifyOtp(email, code) : { token: 'demo', expiresAt: new Date(Date.now() + 3600000).toISOString(), user: { id: 'demo-user', email, displayName: email.split('@')[0], role: 'OWNER' as const, locale, pointsBalance: 128 } }; onAuthenticated(session) }
