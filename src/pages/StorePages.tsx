...
import type { Announcement, Locale, PreorderCampaign, Product, Route, StorefrontData } from '../domain/types'
...
export function StorefrontPage({ data, locale, navigate, addToCart, favoriteIds, toggleFavorite, favoritesOnly = false }: Common & { favoritesOnly?: boolean }) {
  ...
  return <div className="page storefront-page">
    ...
    <div className="catalog-toolbar" data-animate-item><label className="search-field"><MagnifyingGlass size={21} aria-hidden="true" /><span className="sr-only">{text(locale, 'ค้นหาสินค้า', 'Search products')}</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={text(locale, 'ค้นหาสินค้าและหมวดหมู่', 'Search products and categories')} /></label>
    {!favoritesOnly && <div className="category-scroll">{data.categories.filter((item) => item.active).map((item) => <button type="button" key={item.id} className={category === item.id ? 'is-active' : ''} onClick={() => setCategory(item.id)}>{text(locale, item.nameTh, item.nameEn)}</button>)}</div>}</div>
...
function ProductCard({ product,campaign,serverTime, locale, favorite, onOpen, onAdd, onFavorite }: { product: Product;campaign?:PreorderCampaign;serverTime:string; locale: Locale; favorite: boolean; onOpen: () => void; onAdd: () => void; onFavorite: () => void }) {
  ...
  return <article className="product-card" data-animate-item>
    ...
    <div className="product-content"><button type="button" className="product-title" onClick={onOpen}>{text(locale, product.titleTh, product.titleEn)}</button><p>{product.type === 'PREORDER' ? text(locale, `มัดจำ ฿${product.deposit.toLocaleString()}`, `Deposit ฿${product.deposit.toLocaleString()}`) : text(locale, `ได้ ${product.points} แต้ม`, `Earn ${product.points} points`)}</p>{campaign&&<PreorderCountdown closeAt={campaign.closeAt} serverTime={serverTime} locale={locale}/>}<div className="product-price"><strong>฿{product.price.toLocaleString()}</strong><button type="button" className="add-button" onClick={onAdd} disabled={remaining < 1||Boolean(campaign&&campaign.status!=='OPEN')} aria-label={text(locale, `เพิ่ม ${product.titleTh}`, `Add ${product.titleEn}`)}><Plus size={21} /></button></div></div>
  </article>
}
  ...
  return <div className="page product-page">
    ...
    <div className="product-detail-grid">
      ...
      <section data-animate-item><div className="eyebrow-row"><span className="stock-pill">{product.type === 'PREORDER' ? 'Pre-order' : text(locale, 'พร้อมส่ง', 'Ready stock')}</span><button className={`icon-button ${favorite ? 'is-favorite' : ''}`} type="button" onClick={() => toggleFavorite(product.id)} aria-pressed={favorite} aria-label={text(locale, 'สลับรายการโปรด', 'Toggle favorite')}><Heart size={21} weight={favorite ? 'fill' : 'regular'} /></button></div><h1>{text(locale, product.titleTh, product.titleEn)}</h1><p className="lead">{text(locale, product.descriptionTh, product.descriptionEn)}</p>
        {campaign && <div className="campaign-note"><strong>{text(locale, campaign.nameTh, campaign.nameEn)}</strong><span>{text(locale, `คาดว่าถึง ${new Date(campaign.expectedArrival).toLocaleDateString('th-TH')}`, `Expected ${new Date(campaign.expectedArrival).toLocaleDateString('en-US')}`)}</span><PreorderCountdown closeAt={campaign.closeAt} serverTime={data.serverTime} locale={locale}/></div>}
...
function PreorderCountdown({closeAt,serverTime,locale}:{closeAt:string;serverTime:string;locale:Locale}){const offset=useMemo(()=>new Date(serverTime).getTime()-Date.now(),[serverTime]);const [seconds,setSeconds]=useState(()=>Math.max(0,Math.ceil((new Date(closeAt).getTime()-(Date.now()+offset))/1000)));useEffect(()=>{const tick=()=>setSeconds(Math.max(0,Math.ceil((new Date(closeAt).getTime()-(Date.now()+offset))/1000)));tick();const timer=window.setInterval(tick,1000);return()=>window.clearInterval(timer)},[closeAt,offset]);if(seconds<=0)return <span className="preorder-countdown is-closed">{text(locale,'ปิดรับแล้ว','Closed')}</span>;const days=Math.floor(seconds/86400),hours=Math.floor(seconds%86400/3600),minutes=Math.floor(seconds%3600/60),secs=seconds%60;return <span className="preorder-countdown"><span>{text(locale,'ปิดรับใน','Closes in')}</span><strong>{days>0?`${days}${text(locale,'วัน','d')} `:''}{String(hours).padStart(2,'0')}:{String(minutes).padStart(2,'0')}:{String(secs).padStart(2,'0')}</strong></span>}
